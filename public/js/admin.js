...
function renderSOSAlerts() {
    ...
    // Update badges
    [badge, homeBadge].forEach(b => {
        if (b) {
            if (activeCount > 0) { b.style.display = 'inline'; b.textContent = activeCount; }
    ...
    function buildAlertCard(alert, compact) {
        ...
        return `<div onclick="openSOSDetailsModal('${alert.id}')" style="display:flex;align-items:center;gap:${compact ? '10px' : '12px'};padding:${pad};margin-bottom:${compact ? '8px' : '10px'};background:${cardBg};border:1px solid ${cardBorder};border-radius:12px;${isActive ? 'animation:sosAlertPulse 2s infinite;' : ''};cursor:pointer;transition:all 0.2s;" onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(239, 68, 68, 0.1)';" onmouseout="this.style.transform='none'; this.style.boxShadow='none';">
            <div style="width:${sz};height:${sz};border-radius:10px;background:${isActive ? '#ef4444' : '#94a3b8'};color:white;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:${compact ? '0.85rem' : '1rem'};flex-shrink:0;">${alert.userPhoto ? `<img src="${alert.userPhoto}" style="width:100%;height:100%;border-radius:8px;object-fit:cover;">` : initial}</div>
    ...
    if (homeContainer) {
        ...
        if (activeAlerts.length > 0) {
            ...
            if (homeCard && window.isSupervisor) homeCard.classList.remove('hidden');
        } else {
    ...
    if (resolved.length === 0) {
        ...
        container.innerHTML = resolved.map(alert => {
            ...
            return `<div style="display:flex;align-items:flex-start;gap:12px;padding:14px;margin-bottom:10px;background:#fafafa;border:1px solid rgba(0,0,0,0.06);border-radius:10px;">
                <div style="width:40px;height:40px;border-radius:8px;background:#94a3b8;color:white;display:flex;align-items:center;justify-content:center;font-weight:700;flex-shrink:0;">${alert.userPhoto ? `<img src="${alert.userPhoto}" style="width:100%;height:100%;border-radius:8px;object-fit:cover;">` : initial}</div>
                <div style="flex:1;min-width:0;">
    ...
    if (modal && content) {
        ...
        content.innerHTML = `
            <div style="text-align: center; margin-bottom: 24px;">
                <div style="width: 80px; height: 80px; border-radius: 20px; background: #ef4444; color: white; display: flex; align-items: center; justify-content: center; font-size: 2.5rem; font-weight: 800; margin: 0 auto 16px; box-shadow: 0 10px 25px rgba(239, 68, 68, 0.3);">
                    ${alert.userPhoto ? `<img src="${alert.userPhoto}" style="width: 100%; height: 100%; border-radius: 20px; object-fit: cover;">` : initial}
                </div>
